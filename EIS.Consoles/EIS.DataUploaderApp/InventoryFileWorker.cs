...
{
    public class InventoryFileWorker
    {
        private readonly string _connectionString;
        private readonly InventorySettingRepository _settingRepository;

        public InventoryFileWorker(string connectionString)
        {
            _connectionString = connectionString;
            _settingRepository = new InventorySettingRepository(connectionString);
        }
        ...
        public void ReadAndParsedFile()
        {
            ...
            var inventoryFileSettings = _settingRepository.GetInventoryFileSettings(systemDateTime);

            foreach (var fileSetting in inventoryFileSettings)
            {
                try
                {
                    ...
                    if (downloadedFilePath == null)
                    {
                        ...
                        _settingRepository.SetInventoryNextUploadDate(fileSetting.VendorId, systemDateTime.AddDays(1));
                        continue;
                    }

                    // let's create file manager
                    fileSetting.ResultDate = DateTime.Now.Date;
                    ...
                    {                        
                        // set the next read date for the inventory file   
                        _settingRepository.SetInventoryNextUploadDate(t.Result.OwnerId, systemDateTime.AddDays(1));

                        // if it has no error update the main products table with the parsed product data file
                        if (!t.Result.HasError)
                        {
                            var inventoryService = new InventoryFileService(t.Result, _connectionString);
                ...
                {
                    Console.Error.WriteLine(string.Format("Error in parsing inventory file for vendor ID: {0} Error message: {1}", fileSetting.VendorId, ex.Message));
